...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT * from `coordinador_all` where id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
...
<div class="card card-outline card-info">
	<div class="card-header">
		<h3 class="card-title"><?php echo isset($id) ? "Actualizar ": "Registrar Nuevo " ?> Coordinador</h3>
	</div>
	<div class="card-body">
		<form action="" id="assembly-form">
			...
			<div class="form-group">
                ...
                <input name="telefono" id="" class="form-control form no-resize" value="<?php echo isset($telefono) ? $telefono : ''; ?>" />
			</div>
			<div class="form-group">
				<label for="grado" class="control-label">Grado</label>
                <input name="grado" id="" class="form-control form no-resize" value="<?php echo isset($grado) ? $grado : ''; ?>" />
			</div>
		</form>
	</div>
...
<script>
	$(document).ready(function(){
		$('#assembly-form').submit(function(e){
			...
			start_loader();
			$.ajax({
				url:_base_url_+"classes/Master.php?f=save_coordinador",
				...
				success:function(resp){
					if(typeof resp =='object' && resp.status == 'success'){
						location.href = "./?page=coordinador_all";
